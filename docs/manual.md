...
Give Marim the JDBC (Java Database Connectivity) URL of the database the data source refers to through the keyword `url`.
The table below shows the JDBC driver Marim bundles for which dialect, and gives links to the sections in their documentations that discusses their URL formats.
...
##### URLs and property values sources
...
The same applies to the keyword `variable`. In this case, however, Marim will set the content of a environment variable as the URL of a data source or the value of a property. 
...
and the content of the `/var/run/secrets/dvd_rental/password` file as the value of the `password` property.

<pre><code class="language-marim">source DvdRental
    dialect  postgresql
    url      variable "DVD_RENTAL_URL"
    property "user"     "postgres"
    property "password" file "/var/run/secrets/dvd_rental/password"
</code></pre>
...
The specification of the data source in which a query will be executed can be provided in the query specification. Thus, the snippet below is equivalent to the previous one.
